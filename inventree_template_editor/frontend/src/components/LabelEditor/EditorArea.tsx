...
import { LabelEditorState, useLabelEditorState, useLabelEditorStore } from "./LabelEditorContext";
...
export const EditorArea = () => {
  ...
  const handleDrag = useCallback(
    (clientX?: number, clientY?: number) => {
      ...
      const [canvasWidth, canvasHeight] = [editor.canvas.getWidth(), editor.canvas.getHeight()];
      if (vpt) {
        ...
        if (zoom < canvasHeight / pageHeight) {
          ...
          if (vpt[5] >= SPACE_Y * zoom) {
            vpt[5] = SPACE_Y * zoom;
          } else if (vpt[5] < canvasHeight - pageHeight * zoom - SPACE_Y * zoom) {
  ...
  const zoomToFit = useCallback(() => {
    if (!editor) return;
    const [width, height] = [editor.canvas.getWidth(), editor.canvas.getHeight()];

    const minZoom = Math.min(width / (pageWidth + 10), height / (pageHeight + 10));
  ...
  useEvents(
    ...
    (on) => {
      ...
      on("mouse:wheel", (event) => {
        ...
        editor.canvas.zoomToPoint(new FabricPoint(event.e.offsetX, event.e.offsetY), new_zoom);
      ...
      on("object:added", (e) => {
        if (editorState.current.ignoreObjects.has(e.target as FabricObject)) return;
      ...
      const autoSwitchPanel = (e: Partial<TEvent<TPointerEvent>>) => {
        ...
        } else if (selectedObjects.length !== 1 || "group" in selectedObjects[0]) {
      ...
      on("object:scaling", (e) => {
        ...
        if (settings.snap["grid.enable"]) {
          let gridSize = unitToPixel(settings.grid["size.size"], settings.grid["size.unit"]);
          ...
          if (["tl", "ml", "bl"].includes(corner)) {
            ...
            obj.scaleX = (width + obj.left - tl) / (obj.width + obj.strokeWidth);
            obj.left = tl;
          ...
          if (["tl", "mt", "tr"].includes(corner)) {
            ...
            obj.scaleY = (height + obj.top - tt) / (obj.height + obj.strokeWidth);
            obj.top = tt;
      ...
      on("object:moving", (e) => {
        ...
        const obj = e.target as FabricObject;
        const gridSize = unitToPixel(settings.grid["size.size"], settings.grid["size.unit"]);

        if (Math.round((obj.left! / gridSize) * 1) % 1 === 0 && Math.round((obj.top! / gridSize) * 1) % 1 === 0) {
          obj
            .set({
  ...
  useEvents(
    ...
    (on) => {
      on("keyup", (event) => {
        ...
        if (/INPUT|SELECT|TEXTAREA/i.test((event.target as any).tagName)) return;
      ...
      on("keydown", (event) => {
        ...
        const move = (direction: [number, number]) => {
          ...
          const gridSize = unitToPixel(gridSettings["size.size"], gridSettings["size.unit"]);
    ...
    return labelEditorStore.subscribe((s, ps) => {
      if (
        Object.entries(s.pageSettings).some(
          ([k, v]) => v !== ps.pageSettings[k as keyof LabelEditorState["pageSettings"]],
        )
      ) {
        setPageSettings(s);
      }
  ...
  return <FabricJSCanvas onReady={customOnReady} className={classes.editorCanvas} />;
};
