...
export const getCustomFabricBaseObject = <T extends new (...args: any[]) => {}>(Base: T, customFields?: string[]) => {
  const fields = ["positionUnit", "sizeUnit", "strokeWidthUnit", "name", ...(customFields || [])];
...
export const c = (n: number, u: string) => Math.round((pixelToUnit(n, u) + Number.EPSILON) * 10 ** 15) / 10 ** 15 + u;
...
export const styleHelper = {
  ...
  size: (object) => [`width: ${c(object.width, object.sizeUnit)};`, `height: ${c(object.height, object.sizeUnit)};`],
  rotation: (object) =>
    object.angle !== 0 ? ["transform-origin: top left;", `transform: rotate(${object.angle}deg);`] : [],
  ...
  border: (object) => {
    ...
    return [`outline: ${c(object.strokeWidth, object.strokeWidthUnit)} solid ${object.stroke};`];
  },
